...
from .models import Classroom, Attendance, StudentInfo, StudentClassDetails
import warnings
...
# Sửa đường dẫn model_dir - sử dụng đường dẫn tương đối từ thư mục face_attendance
...
def insert_attendance(id_classroom, student_id):
    ...
    # Sửa: Sử dụng StudentClassDetails để lấy danh sách sinh viên trong lớp
...
def main(id_subject):
    ...
    # Sửa đường dẫn model - sử dụng đường dẫn tương đối từ thư mục face_attendance
    current_dir = os.path.dirname(os.path.abspath(__file__))
    CLASSIFIER_PATH = os.path.join(current_dir, 'Models', 'facemodel.pkl')
    FACENET_MODEL_PATH = os.path.join(current_dir, 'Models', '20180402-114759.pb')
