...
from main.models import StaffInfo, StudentClassDetails, Classroom
...
# Sửa import: reg.py nằm trong face_attendance, không phải main.view.reg
# Nếu đây là Django app với tên 'main', có thể cần thay đổi cấu trúc
# Tạm thời import trực tiếp từ face_attendance
try:
    from .reg import main as reg_main, insert_attendance
except ImportError:
    # Fallback nếu import tương đối không hoạt động
    from face_attendance.reg import main as reg_main, insert_attendance
from main.models import BlogPost

model_test = AntiSpoofPredict(0)
image_cropper = CropImage()

# Sửa đường dẫn model_dir - sử dụng đường dẫn tương đối
import os
...
def live_video_feed2(request, classroom_id):
    print(classroom_id)
    # Sửa: sử dụng reg_main thay vì main (đã import từ reg.py)
    return StreamingHttpResponse(reg_main(classroom_id),
...
def lecturer_mark_attendance_by_face(request, classroom_id):
    ...
    # During testing it's useful to bypass the weekday check so the page can
    # be opened any day. Use environment variable BYPASS_DAY_CHECK=0 to
    # re-enable the original behavior.
    import os as _os
    bypass_day_check = _os.environ.get('BYPASS_DAY_CHECK', '1') == '1'
    if not bypass_day_check:
        if day_of_week_today != classroom.day_of_week_begin:
            return redirect('lecturer_attendance')
